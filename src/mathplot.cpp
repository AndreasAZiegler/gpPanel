...
void mpFXYVector::SetData( const std::deque<double> &xs,const std::deque<double> &ys)
{
    ...
    if (xs.empty())
    {
        ...
        for(std::deque<double>::const_iterator it=xs.begin();it!=xs.end();it++)
        {
            if (*it<m_minX) m_minX=*it;
            if (*it>m_maxX) m_maxX=*it;
        }
        for (std::deque<double>::const_iterator it=ys.begin();it!=ys.end();it++)
        {
